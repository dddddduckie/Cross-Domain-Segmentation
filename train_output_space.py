...
LAMBDA_ADV_TARGET_1 = 0.0
LAMBDA_ADV_TARGET_2 = 0.01
...
def train(model, discriminator, optimizer, source_data_iter,
    ...
    for i in range(start_iter, parser.max_iter + 1):
        ...
        output_map1 = D1(F.softmax(output1_target))
        output_map2 = D2(F.softmax(output2_target))
        ...
        loss_adv1_target = compute_loss(output_map1, label_map1, name='mse')
        loss_adv1_target_value += loss_adv1_target.item()
        loss_adv2_target = compute_loss(output_map2, label_map2, name='mse')
        ...
        output_map1 = D1(F.softmax(output1_source))
        output_map2 = D2(F.softmax(output2_source))
        label_map1 = torch.FloatTensor(output_map1.data.size()).fill_(SOURCE_LABEL).to(device)
        label_map2 = torch.FloatTensor(output_map2.data.size()).fill_(SOURCE_LABEL).to(device)
        loss_D1 = compute_loss(output_map1, label_map1, name='mse')
        loss_D1 = loss_D1 / 2
        loss_D1.backward()
        loss_D1_value += loss_D1.item()
        loss_D2 = compute_loss(output_map2, label_map2, name='mse')
        loss_D2 = loss_D2 / 2
        loss_D2.backward()
